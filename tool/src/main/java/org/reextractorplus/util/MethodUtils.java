        ...
        if (body != null) {
            ...
            if (statements.size() == 1) {
                ...
                if (statement.toString().startsWith("return ")) {
                    ...
                    Visitor visitor = new Visitor((CompilationUnit) methodDeclaration.getRoot(), "", null);
                    statement.accept(visitor);
                    boolean parameterUsed = false;
                    for (SingleVariableDeclaration parameter : parameters) {
                        for (LeafExpression variableExpression : visitor.getVariables()) {
                            if (variableExpression.getString().equals(parameter.getName().getIdentifier())) {
                                parameterUsed = true;
                                break;
                            }
                        }
